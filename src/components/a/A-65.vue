<template>
    <div class="A65 text-center main-a lengthways-a">
      <p class="top-name-p width-710">A-65</p>
      <p align="center" >贵 州 省 高 速 公 路 建 设 项 目 </p>
      <p>黔西南州兴义环城高速公路</p>
      <h3>承包人施工设备退场申报审批表</h3>
      <p class="title-p-right margin-auto width-710">责查表-21</p>
      <div class="public-tilte flex-r margin-auto width-710">
        <div>
          <p>合 同 号：<input type="text" v-model="obj.m2"></p>
          <p>起迄桩号：<input type="text" v-model="obj.m3"></p>
        </div>
        <div>
          <p>申报工点：<input type="text"></p>
          <p>申报日期：
      ...
      <div align="center">
        <table border="1" cellspacing="0" class="public-tabel" >
          ...
          <tr v-for="(val,index) in controlArr" :key="index">
            ...
            <td width="58" valign="center" ><p align="center"><input type="text" v-model="val.s4"></p></td>
            ...
            <td width="82" valign="center" ><p align="center"><input type="text" v-model="val.s7"></p></td>
      ...
      <p class="bot-hint-p width-710">注：审批后该表内容应进入汇总表汇总，并与审批后的汇总表一并报驻地办备案。并编入工程月报。 <br /><br />
        现场负责人：<input type="text" v-model="obj.m1" class="input-80">  监理工程师：<input type="text" v-model="nameList1[1].fullName" disabled class="input-80">  日期：
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //查询
      getData() {
        this.axios.get('/HiQuality/A65/findA65ById', {
          params: {
      ...
      addData() {
        ...
        this.axios.post("/HiQuality/A65/saveA65", this.$qs.stringify({
          a65:JSON.stringify(this.obj),
      ...
      updateData() {
        this.obj.subList = this.controlArr;
        this.axios.post('/HiQuality/A65/updateA65', this.$qs.stringify({
          a65:JSON.stringify(this.obj)
        }))
          .then(res => {
            console.log("修改接口")
      ...
      //删除
      deleteData() {
        this.axios.get('/HiQuality/A65/deleteA65', {
          params: {
            id: this.id
          }
        }).then(res => {
