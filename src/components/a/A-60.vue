<template>
  <div class="A60 text-center main-a lengthways-a ">
    <p class="top-name-p width-682">A-60</p>
    <p align="center">贵 州 省 高 速 公 路 建 设 项 目 </p>
    <p>黔西南州兴义环城高速公路</p>
    <h3>承包人现场施工设备核查汇总表</h3>
    <p class="title-p-right margin-auto width-682">责查表-16</p>
    <div class="public-tilte flex-r margin-auto width-682">
      <div>
        <p>合 同 号：<input type="text"></p>
      </div>
      <div>
        <p>核查日期：
    ...
    <p class="bot-hint-p width-682">注：汇总责查表-14。总监办每月应核查不少于2次，并及时送项目公司备案，监理核查还应进入监理月报。 <br/><br/>
      监理员：<input type="text" v-model="nameList1[1].fullName" disabled class="input-105"> 监理工程师：<input type="text"
                                                                                                      v-model="nameList2[1].fullName"
                                                                                                      disabled
                                                                                                      class="input-105">承包人现场负责人：<input
        type="text" v-model="obj.m3" class="input-105">
    </p>
    <div class="btn">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //查询
      getData() {
        this.axios.get('/HiQuality/A60/findA60ById', {
          params: {
      ...
      addData() {
        ...
        this.axios.post("/HiQuality/A60/saveA60", this.$qs.stringify({
          a60: JSON.stringify(this.obj),
      ...
      updateData() {
        this.obj.subList = this.controlArr;
        this.axios.post('/HiQuality/A60/updateA60', this.$qs.stringify({
          a60: JSON.stringify(this.obj)
        }))
          .then(res => {
            console.log("修改接口")
      ...
      //删除
      deleteData() {
        this.axios.get('/HiQuality/A60/deleteA60', {
          params: {
            id: this.id
          }
        }).then(res => {
