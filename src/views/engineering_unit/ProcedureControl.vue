          ...
          <el-col :span="24">
            <div>
              <div
                ...
                @click.stop="tabType(index+1)"
              >
                {{index > 0 ? item.projPartsName : item.unitProjName}}
                <span class="split-line">/</span>
              </div>
              <span class="unit-name">
                {{getLable(breadCrumbs)}}
              </span>
            </div>
            <!--   <span
                 class="unit-name active-unit"
                 v-show="subType>1"
                 @click.stop="tabType(2)">
                 /分部工程
               </span>
               <span
                 class="unit-name active-unit"
                 v-show="subType>2"
                 @click.stop="tabType(3)">
                 /分项工程
               </span>
               <span
                 class="unit-name active-unit"
          ...
          <div class="list relative" v-if="subType==2">
            <div class="list-item" v-for="(item,index) in branchList" :key="index"
              ...
              <div class="list-item-line "
                   @click.stop="showSubentry(3,item.child,item.id,item)">
                <div class="list-item-left-icon">
                  <i class="iconfont ">&#xe902;</i>
                </div>
        ...
        >
          <div class="list relative" v-if="subType==3">
            ...
            <div class="list-item" v-for="(item,index) in subentryList" :key="index">
              <div class="list-item-line "
                   @click.stop="showSubentryList(4,item.child,item.id,item)">
                <div class="list-item-left-icon">
                  <i class="iconfont ">&#xe902;</i>
                </div>
                <div class="line-center">
                ...
                <div class="line-right">
                  <div class="line-right-icon"
                    ...
                    <i class="iconfont  black-icon "
                       @click.stop="getProProject(item,3)">&#xe707;
                    </i>
                  </div>
                  <div class="line-right-icon">
                    <i class="iconfont  black-icon ">&#xe602;</i>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      getProcedureControl() {
        return this.$store.state.procedureControl
      }
    },
    watch: {
      getProcedureControl(val) {
    ...
    mounted() {
      this.breadCrumbs = this.$store.state.procedureControl;
    },
    methods: {
      /**
       * 获取默认提示信息
      ...
      //显示分项
      showSubentry(type, list, id, obj) {
        this.subType = type;
        this.subentryList = list;
        let arr = this.$store.state.procedureControl;
        arr[1] = obj;
        this.$store.commit("setProcedureControl", arr);
        this.getK02(id, 1)
      },
      //显示项目分部
      showBranch(type, id, obj) {
        ...
        this.$store.commit("setProcedureControl", [obj]);
        this.getAllSub(id)
      },
      //获取分项工程表头信息
      getK01(id, isOk) {
      ...
      //显示子分项
      showSubentryList(type, list, id,obj) {
        this.subType = type;
        this.getK01(id, 1);
        this.subChildentryList = list;
        let arr = this.breadCrumbs;
        arr[2] = obj;
        this.$store.commit("setProcedureControl",arr);
      },
      /**
      ...
      tabType(index, id) {
        this.subType = index;
        let arr = [];
        this.breadCrumbs.forEach((item, bIndex) => {
          if (bIndex < index - 1) {
            arr.push(item);
          }
        });
    ...
    beforeRouteLeave(to, from, next) {
      ...
      console.log(to.path);
      console.log("-------")
      if (to.path == "/procedure_control/process_list/index") {
        console.log("需要换成")
        from.meta.keepAlive = true;
      } else {
        from.meta.keepAlive = false;
        console.log("不用缓存");
        this.$store.commit("setProcedureControl",[])
